...
if __name__ == "__main__":
    ...
    if args.dataset == 'pubmed':
        dataset_details = utils.load_pubmed()
        model_params = (3, 256, 4)
        batch_size = 1024
        partition = 1000
        fanout = 10
    elif args.dataset == 'ogbn-arxiv':
        dataset_details = utils.load_data('ogbn-arxiv')
        model_params = (2, 512, 2)
        batch_size = 1024
        partition = 5000
        fanout = 10
    elif args.dataset == 'reddit':
        dataset_details = utils.load_reddit()
        model_params = (4, 1024, 2)
        batch_size = 1024
        partition = 5000
        fanout = 10
    elif args.dataset == 'ogbn-products':
        dataset_details = utils.load_data('ogbn-products')
        model_params = (3, 256, 2)
        batch_size = 1024
        partition = 3000
        fanout = 5
    elif args.dataset == 'orkut':
        dataset_details = utils.load_data('orkut')
    ...
    data = (
        dataset_details.num_classes,
        dataset_details.train_idx,
        dataset_details.val_idx,
        dataset_details.test_idx,
    )
    graph = dataset_details[0]
    ...
    f = dataset['fanout']
    if dataset in ["ogbn-arxiv", 'ogbn-papers100M', 'orkut'] :
