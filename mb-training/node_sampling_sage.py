...
# Define the header
header = ['dataset', 'model', 'batch_size', 'num_partition', 'fanout', 'shuffle', 'use_ddp', 'gpu_model', 'n_gpu', 'n_layers', 'n_hidden', 'test_accuracy', 'epoch_time', 'n_50_accuracy', 'n_50_tta', 'n_50_epoch', 'n_200_accuracy', 'n_200_tta', 'n_200_epoch']
...
def train(
    ...
    try:
        if proc_id == 0:
            ...
            data = ds, model_name, batch_size, num_partitions, fanout, shuffle, use_ddp, gpu_model, n_gpu, n_layers, n_hidden, best_test_acc, epoch_time, n_50_accuracy, n_50_tta, n_50_epoch, n_200_accuracy, n_200_tta, n_200_epoch
            print(data)
...
def run(proc_id, nprocs, devices, g, data, mode, params):
    try:
        ...
        dist.init_process_group(
            backend="nccl",
            init_method="tcp://127.0.0.1:12352",
            world_size=nprocs,
            rank=proc_id,
        )
        ...
        model = NSGraphSAGE(in_feats, n_hidden, n_classes, n_layers, dropout=0.3, activation=F.relu).to(device)
        ...
        for s in shuffle:
            for d in use_ddp:
                for n_g in n_gpu:
                    ...
                    print('Running parameters : ',ds, b, p, f, s, d, n_g, l, h)
                    mp.spawn(run, args=(nprocs, devices, g, data, args.mode, (ds, b, p, f, s, d, n_g, l, h)), nprocs=nprocs, join=True)
                    i+=1    