...
def train(
    ...
    print(device, 'saint sampler') 
    ds, batch_size, num_partitions, mode, budget, shuffle, use_ddp, n_gpu, n_layers, n_hidden, num_heads = params 
...
def run(proc_id, nprocs, devices, g, data, mode, params):
    ...
    try:
        ds, batch_size, num_partition, mode, budget, shuffle, use_ddp, n_gpu, n_layers, n_hidden, num_heads = params 
...
if __name__ == "__main__":
    ...
    if args.dataset == 'pubmed':
        ...
        model_params = (2, 128, 1)  # Correctly copied as specified
        batch_size = 128
        budget = 15000
        partition = 5000
    ...
    for s in shuffle:
        for d in use_ddp:
            ...
            print('Running parameters : ', args.dataset, b, p, m, bu, s, d, args.gpu, l, h, head)
            mp.spawn(run, args=(nprocs, devices, g, data, args.mode, (args.dataset, b, p, m, bu, s, d, args.gpu, l, h, head)), nprocs=nprocs, join=True)
            i += 1
